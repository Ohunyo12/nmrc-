...
export class UnifiedUnderwritingStandardService {
  ...
  getApprovalGroup(productId: number, productClassId: number, operationId: number): Observable<any> {
    ...
    const params = {
      ...
      productClassId: productClassId.toString(),
      operationId: operationId.toString()
    };

    return this.http.get(`${AppConstant.API_BASE}loan/approval-group-mapping-flow`, { params }).pipe(
        map((res: any) => res),
        catchError(err => observableThrowError(() => err.error || 'Server error'))
      );
  }
