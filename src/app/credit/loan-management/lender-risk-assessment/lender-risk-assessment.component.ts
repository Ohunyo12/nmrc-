...
@Component({
  selector: 'app-lender-risk-assessment',
  templateUrl: './lender-risk-assessment.component.html',
  styles: []
})

export class LenderRiskAssessmentComponent implements OnInit {
  ...
  fetchLoansForReview(rows: number = 5): void {
    ...
    this.underwritingService.getReviewedSumForNmrcApproval().subscribe(
      response => {
  ...
  viewLoanDetails(refinanceNumber: string) {
    ...
    if (this.selectedLoan === refinanceNumber) {
      ...
      this.underwritingService.getReviewedForApproval(refinanceNumber).subscribe(
        (response) => {
  ...
  approveReviewLoans() {
    if (!this.selectedLoanDetail) {
      swal("FinTrak Credit 360", "Please select a loan to proceed with final approval.", "error");
      return;
    }
    ...
    swal({
      title: "Confirm Final Loan Approval",
      text: "Are you sure you want to approve this loan for tranching?",
      type: "warning",
      showCancelButton: true,
      allowOutsideClick: false,
      confirmButtonColor: "#3085d6",
      ...
      if (confirmed) {
        ...
        this.underwritingService.approveFinalReviewedLoan([loanId]).subscribe({
          next: (response) => {
            this.loadingService.hide();
            console.log("API Response:", response);

            if (response.success) {
              swal("FinTrak Credit 360", "Loan successfully approved for tranching!", "success").then(() => {
                this.fetchLoansForReview();
                this.selectedLoanDetail = null;
              });
            } else {
              swal("FinTrak Credit 360", response.message || "Failed to approve the loan. Please try again.", "error");
            }
          },
          error: (error) => {
            this.loadingService.hide();
      ...
      if (reason === 'cancel') {
        ...
        swal("FinTrak Credit 360", "Final loan approval cancelled.", "info");
      }
    });
  }
  ...
  disApproveReviewLoans() {
    if (!this.selectedLoanDetail) {
      swal("FinTrak Credit 360", "Please select a loan to proceed with disapproval.", "error");
      return;
    }

    console.log("Disapproving Loan:", this.selectedLoanDetail);

    const loanId = this.selectedLoanDetail.id;

    swal({
      title: "Confirm Loan Disapproval",
      text: "Are you sure you want to disapprove this loan? This will prevent it from proceeding to tranching.",
      type: "warning",
      showCancelButton: true,
      allowOutsideClick: false,
      confirmButtonColor: "#3085d6",
      ...
      if (result.isConfirmed) {
        ...
        this.underwritingService.disapproveFinalReviewedLoan([loanId]).subscribe({
          next: (response) => {
            this.loadingService.hide();
            console.log("API Response:", response);

            if (response.success) {
              swal("FinTrak Credit 360", "Loan successfully disapproved.", "success").then(() => {
                this.fetchLoansForReview();
                this.selectedLoanDetail = null;
              });
            } else {
              swal("FinTrak Credit 360", response.message || "Failed to disapprove the loan. Please try again.", "error");
            }
          },
          error: (error) => {
            this.loadingService.hide();
  ...
  sendForTranching() {
    ...
    this.selectedLoanDetail = null;
    if (!selectedLoan) {
      swal("FinTrak Credit 360", "Please select a loan to send for tranching.", "error");
      return;
    }
    ...
    swal({
      ...
      text: "Are you sure you want to send the Batch loan for Tranch",
      type: "warning",
      showCancelButton: true,
      allowOutsideClick: false,
      confirmButtonColor: "#3085d6",
      ...
      if (confirmed) {
        ...
        this.underwritingService.sendLoanForTranch(refinanceNumber).subscribe({
          next: (response) => {
            this.loadingService.hide();
            console.log("API Response:", response);

            if (response.success) {
              swal("FinTrak Credit Risk 360", "Batch Loan successfully approved and sent for tranching!", "success").then(() => {
                // Clear approved loan
                this.loanDetails = [];
