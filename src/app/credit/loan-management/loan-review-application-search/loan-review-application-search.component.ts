...
@Component({
  selector: 'app-loan-review-application-search',
  templateUrl: './loan-review-application-search.component.html',
})
export class LoanReviewApplicationSearchComponent implements OnInit, OnDestroy {
  ...
  getTransacDynamics(loanApplicationId): void {
      ...
      this.camService.getLMSTransacDynamics(loanApplicationId).subscribe((response) => {
  ...
  getConditionPrecident(loanApplicationId): void {
      ...
      this.loadingService.show();
      this.subscriptions.add(
      this.camService.getLoanConditionPrecidents(loanApplicationId).subscribe((response) => {
  ...
  submitForm(form) { 
    ...
    let body = {
      ...
      this.loadingService.show();
      this.subscriptions.add(
      this.loanService.loanReviewApplicationSearch(body).subscribe((response) => {
      ...
      swal({
          ...
          buttonsStyling: true,
           ...
           let body = {
              ...
              loanReviewApplicationId:__this.application.loanApplicationId
          };
          __this.subscriptions.add(
          __this.loanService.loanApplicationLmsCancellationRequest(body).subscribe((response) => {
  ...
  popoverSeeMore() {
          ...
          ////console.log(data);
          this.subscriptions.add(
          this.reportServ.getGeneratedOfferLetterLMS(data.applicationRefNumber).subscribe((response) => {
...
//   deleteDocument(row) {
//     this.loadingService.show();
//     this.documentpUloadService.deleteUploadedDocument(row).subscribe((response) => {
//         this.binaryFile = response.result.fileData;
...
getDocumentationSections(showLoadDocumentModal: boolean) {
    this.loadingService.show();
    this.subscriptions.add(
    this.camService.getDocumentSections(this.APPRAISAL_OPERATION_ID, this.application.loanApplicationId).subscribe((response) => {
...
onDocumentSectionChange(sectionId) {
    this.loadingService.show();
    this.subscriptions.add(
    this.camService.getDocumentSection(this.APPRAISAL_OPERATION_ID, this.application.loanApplicationId, sectionId).subscribe((response) => {
...
closeDocumentation() {
  ...
  previewDocumentation(print = false) {
    this.loadingService.show();
    this.camService.getDocumentation(this.APPRAISAL_OPERATION_ID, this.application.loanApplicationId).subscribe((response) => {
        this.documentations = response.result;
        this.loadingService.hide();
        if (print == false) this.displayDocumentation = true;
          //else setTimeout(() => this.print2222(), 1000);
    }, (err) => {
        this.loadingService.hide(1000);
    });
}
...
//   }
  ...
  previewDocumentationLOS(print=false) {
    this.loadingService.show();
    this.camService.getDocumentation(this.OPERATION_ID, this.application.creditAppraisalLoanApplicationId).subscribe((response) => {
        this.documentations = response.result;
        this.loadingService.hide();
        if (print == false) this.displayDocumentation = true;
        //else setTimeout(() => this.print2222(), 1000);
