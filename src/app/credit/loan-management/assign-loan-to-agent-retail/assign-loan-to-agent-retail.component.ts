...
@Component({
  selector: 'app-assign-loan-to-agent-retail',
  templateUrl: './assign-loan-to-agent-retail.component.html'
})
export class AssignLoanToAgentRetailComponent implements OnInit {
...
goForApproval(row, evt) {
  ...
  autoAssign() {
    this.loadingService.show();
    this.camService.autoAssign().subscribe((response: any) => {
  ...
  getAllLoansRecoveredByAgent(accreditedConsultantId, loanAssignId) {
    this.loanOperationService.getAllBulkLoansRecoveredByAgent(accreditedConsultantId, loanAssignId).subscribe((response: any) => {
  ...
  assignMultipleLoansToAgent() {
    ...
    this.loadingService.show(); 
    this.camService.assignMultipleLoansToAgent(this.schemeSelection, this.accreditedConsultantId, date, this.source, assignmentType).subscribe((response: any) => {
        if (response.success === true) {
          this.loadingService.hide(1000);
            this.getAllLoansOperationRecoveryAnalysis();
            this.getBulkRecoveryToAgentAwaitingApproval();
...
getTrail() {
  this.loadingService.show();
  this.subscriptions.add(
          this.camService.getTrailLms(this.recoveryApprovalId, this.recoveryOperationId).subscribe((response: any) => {
