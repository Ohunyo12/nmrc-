...
submitForm(form) {
          ...
          this.loanOperationService.getAllMonthlyInternalRecoveryCommissonByAgents(startDate).subscribe((response: any) => {
  ...
  callResult(){
    this.loanOperationService.GetAllInternalRecoveryAgents(this.searchStartDate).subscribe((response: any) => {
      this.loanOperationApprovalData = response.result;
      
  
      // Call the second function
      this.loanOperationService.getAllMonthlyInternalRecoveryCommissonByAgents(this.searchStartDate).subscribe((response: any) => {
        this.recoveryCommissionData = response.result;
        this.loadingService.hide();
      });
    });
  ...
  computeComission(){
    ...
    if(amoutRecovered > totalRecov){
      swal(`${GlobalConfig.APPLICATION_NAME}`, 'Recovered Amount can not be greater than the total recovery amount', 'error');
      return;
    }

    if(amoutRecovered >= this.recoveryComputationVariablesData.recoveredAmountAbove){
      this.recoveryCommissionForm.controls['commissionRate'].setValue(this.recoveryComputationVariablesData.commissionPayable);
      this.recoveryCommissionForm.controls['commissionPayable'].setValue(this.recoveryComputationVariablesData.commissionPayableLimit);
    }
    if(amoutRecovered <  this.recoveryComputationVariablesData.recoveredAmountBelow){
      const commission = ((amoutRecovered/this.recoveryComputationVariablesData.recoveredAmountBelow)*this.recoveryComputationVariablesData.commissionPayableLimit).toFixed(2);
      this.recoveryCommissionForm.controls['commissionRate'].setValue(this.recoveryComputationVariablesData.commissionPayable);
  ...
  saveDocumentUpload(form, overwrite = false) {
    ...
    saveRecoveryCommissionForm(form, overwrite = false) {
      let body = {
        ...
        recoveryMonth: this.searchStartDate,
      };
        ...
        this.camService.loanRecoveryCommissionInternal(body).subscribe((response: any) => {
          this.loadingService.hide();
            if (response.success == true) {
              ...
              //this.getAllRecoveryCommissonByAgents(form);
              this.loadingService.hide();
              swal(`${GlobalConfig.APPLICATION_NAME}`, response.message, 'success');
              this.callResult();
            }
