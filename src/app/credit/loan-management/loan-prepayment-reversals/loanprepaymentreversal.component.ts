...
@Component({
  selector: 'app-loanprepaymentreversal',
  templateUrl: './loanprepaymentreversal.component.html',
})

export class LoanPrepaymentReversalComponent implements OnInit, OnDestroy {
  totalAmount: any;
...
private subscriptions = new Subscription();
  ...
  getReversalData(loanId){

    this.loanPrepaymentService.getRunningLoanCurrentReversal(loanId)
  ...
  submitForm(formObj) {
    ...
    let bodyObj = {
      ...
      operationTypeId: 216,
      reviewDetails: "Prepament Reversal Operation",
    ...
    if (this.selectedId === null) {
      this.subscriptions.add(
      this.loanPrepaymentService.savePrepaymentReversal(bodyObj).subscribe((res) => {
        if (res.success == true) {
          this.scatterdPayments = [];
