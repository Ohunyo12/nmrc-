...
@Component({
    selector: 'fs-caption-upload',
    templateUrl: 'fs-caption-upload.component.html',
})

export class FsCaptionUploadComponent implements OnInit {
    username: string;
    systemDate: Date;
    collapseForm: boolean = false;
    displayLoanList: boolean = true;
    headerText: string = "Bulk FS Caption Upload";
    pagetitle: string = "Bulk FS Caption Upload";
    ...
    customerDetail: any;
    displayUploadScreen: boolean = false;
    onSelectedCustomerDetailsChanged() {
    this.customerDetail = this.customerSelection;
    this.displayUploadScreen = true;
    }
    ngOnInit() {
    ...
    collapseSearchForm(flag: boolean) {
        ...
        if (flag == true) {
            ...
            this.headerText = "Fs Caption List";
        } else {
            this.headerText = "Fs Caption Bulk Upload";
    ...
    requestFsCaptionBulk() {
        if (this.adActive) {
            this.buttonText = "Uploads Bulk FS Caption";
            this.displayADAuth = true;
            this.bulkUPload = true;
        } else {
            this.adAuthPassCode = "";
            this.uploadBulkFsCaptionFile();
        }
    }

    uploadBulkFsCaptionFile() {
        ...
        if (this.file != undefined || this.file != null) {
            ...
            this.customerService.uploadBulkFsCaptionFile(this.file, body).then(
                (res: any) => {
                    if (res.success == true) {
        ...
        swal({
            ...
            function () {
                ...
                __this.customerService
                    .saveMultipleFsCaptionRequests(__this.uploadSelection)
                    .subscribe(
                        (response) => {
                            if (response.success === true) {
                                this.displayUploadScreen = false;
                                swal(
                                    `${GlobalConfig.APPLICATION_NAME}`,
                                    response.message,
                                    "success"
