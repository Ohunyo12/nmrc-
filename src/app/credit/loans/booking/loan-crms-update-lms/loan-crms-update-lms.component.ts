...
import { GlobalConfig, JobSourceEnum, LMSOperationEnum } from 'app/shared/constant/app.constant';
...
import { LoanReviewApplicationService } from 'app/credit/services/loan-review-application.service';
...
@Component({
  selector: 'app-loan-crms-update-lms',
  templateUrl: './loan-crms-update-lms.component.html',
  styleUrls: ['./loan-crms-update-lms.component.scss']
})
export class LoanCrmsUpdateLmsComponent implements OnInit {
  ...
  activeIndex: any;
  jobSourceId : number;
  readonly OPERATION_ID: number = 48;
...
ngOnDestroy(): void {
  ...
  // @ViewChild(ConditionChecklistComponent) conditionChecklist: ConditionChecklistComponent;
    // @ViewChild(CrmsRegulatoriesComponent) resetCrmsCode: CrmsRegulatoriesComponent;
	// @ViewChild(ApprovalActionsComponent, {static: false}) approvalActionsComponent: ApprovalActionsComponent;
  constructor(
  ...
  getDocumentationSections(showLoadDocumentModal: boolean) {
      ...
      this.camService.getDocumentSections(this.APPRAISAL_OPERATION_ID, this.application.lmsApplicationId).subscribe((response: any) => {
  ...
  onDocumentSectionChange(sectionId) {
      this.loadingService.show();
      this.subscriptions.add(
      this.camService.getDocumentSection(this.APPRAISAL_OPERATION_ID, this.application.lmsApplicationId, sectionId).subscribe((response: any) => {
  ...
  closeDocumentation() {
    ...
    previewDocumentation(print=false) {
      this.loadingService.show();
      this.camService.getDocumentation(this.APPRAISAL_OPERATION_ID, this.application.lmsApplicationId).subscribe((response: any) => {
  ...
  print(): void {
      this.previewDocumentation(true);
          let printTitle = 'CREDIT APPRAISAL - ' + this.application.loanReferenceNumber;
  ...
  updateWorkflowTarget() {
      this.workflowTarget.targetId = this.application.lmsApplicationId;
  ...
  displayApplicationStatusMessage(response) {
      ...
      if (response.stateId == 3)
          swal(`${GlobalConfig.APPLICATION_NAME}`, `The ` + operationName + ` has been Successfully <strong>${response.statusName}</strong>`, 'success');
      else{
