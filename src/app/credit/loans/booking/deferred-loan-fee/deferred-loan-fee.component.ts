...
@Component({
  selector: 'app-deferred-loan-fee',
  templateUrl: './deferred-loan-fee.component.html',
  styleUrls: ['./deferred-loan-fee.component.scss']
})
export class DeferredLoanFeeComponent implements OnInit {
...
// }
	...
	makeDeferredFeeFormVisible = false;
  showFee: any;
	showFeeOptions = [
				{name: 'Yes', value: true},
				{name: 'No', value: false}
  ...
  constructor(
	...
	showDeferredFeeForm(event) {
		if (event == 'true') {
			this.makeDeferredFeeFormVisible = true;
		} else if (event == 'false') {
			this.makeDeferredFeeFormVisible = false;
		}
			//console.log(this.makeDeferredFeeFormVisible === true);
	}
	...
	addApplicationCollection(form) {
		...
		this.applicationCollection.push({
			// loanChargeFeeId: this.collectionId++,
			applicationDetailIdId: this.selectedLoanDetailId,
			...
			chargeFeeName: feeName == null ? 'n/a' : feeName.name,
			description: form.value.description,
			feeAmount: this.isRate == false ? Number(
				form.value.feeAmount.replace(/[,]+/g, "").trim()
			) : feeAmount,
			feeRateValue: this.isRate == true ? form.value.feeAmount : 0,
			isDeferred: true,
			effectiveDate: form.value.effectiveDate,
			feeDependentAmount: this.amount,
		});
		console.log("this.applicationCollection",this.applicationCollection);
