...
@Component({
    templateUrl: 'contingent-release.component.html',
    styles: [`
    .remove-btn{
        color:#bbb;
        font-size:27px;
...
export class ContingentFacilityReleaseComponent implements OnInit {
    ...
    contingentLoanInput: contingentLoanInputModel;
    ...
    verifyLegalContingentCode(){
        var x = this.loanBookingForm.value.legalContingentCode;
        this.loanBookingService.verifyLegalContingentCode(x,this.loanSelection.loanApplicationDetailId).subscribe((response: any) => {
            if (response.data == true) {
                    swal(
                        'Fintrak Credit 360', 
                        'The Legal Reference Number ('+x+') already exist.', 
                        'warning'
                        );
                    this.loanBookingForm.controls['legalContingentCode'].setValue(null);
                this.onCustomerAccountChange(this.loanBookingForm.value.casaAccountId);
            }else {
                this.loanBookingForm.controls['legalContingentCode'].setValue(x);
            }
        }, err => { });
    }
    ...
    addToList(form){
        ...
        else
        {
            ...
            swal(
                'Fintrak Credit 360',
                'Beneficiary total amount cannot be greater than the loan amount',
                'info'
                );
        }
    ...
    getCollections() { 
        if (this.isContingentLoan) {
            ...
            this.contingentLoanInput = new contingentLoanInputModel
                (
                ...
                this.loanBookingForm.value.legalContingentCode,
            )
        }
        ...
        this.loanCollection = new LoanModel(
            ...
            this.contingentLoanInput,                               // contingentLoanInput
    ...
    maintainLine(){ 
        ...
        swal({
            title: 'Are you sure?',
            text: 'The customer has met all desired requirement to maintain line?',
            type: 'question',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
            confirmButtonText: 'Yes',
            cancelButtonText: 'No, Just Maintain!',
            ...
            if (dismiss === 'cancel') {
               collections.approvedLineStatusId = 2;
              // console.log('collections',collections);
                __this.loadingSrv.show();
                __this.loanBookingService.maintainFacilityLine(collections)
                .subscribe((res) => {
    ...
    getAvailedMultipleApplicationsForBooking() {
        this.loadingSrv.show();
        this.loanBookingService.getAvailedContingentFacilityForRelease().subscribe((res) => {
    ...
    loadCustomerApplicationDetails(data) {
        ...
        if (this.loanSelection.bookingAmountRequested > this.loanSelection.customerAvailableAmount && !this.isInEditMode) {
            ...
            const legalContingentCode = this.loanBookingForm.controls['legalContingentCode'];
            ...
            if (this.loanSelection.productTypeId === ProductTypeEnum.Revolving) {
                ...
                if (!isNullOrUndefined(legalContingentCode)) {
                    legalContingentCode.setValidators(Validators.required);
                    legalContingentCode.updateValueAndValidity;
                }
    ...
    onSelectedLoanChange(event) {
        this.loanSelection = {};
        this.loanSelection = event;
        // this.currentApprovalLevelId = event.data.currentApprovalLevelId;
        this.approvalStatusId = event.data.approvalStatusId;
        if(event.data.approvalStatusId == ApprovalStatusEnum.Referred)
        {
            ...
            this.isLoanInfoDisabled = false;
            this.getDrawdownMemoHtml(event.data.loanBookingRequestId);
        }
