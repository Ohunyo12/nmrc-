...
@Component({
  selector: 'app-los-completed-credit-documentation',
  templateUrl: './los-completed-credit-documentation.component.html',
    styles: [`
    .remove-btn{
        color:#bbb;
        font-size:27px;
...
export class LosCompletedCreditDocumentationComponent implements OnInit {
    ...
    submitForm(form) {
        ...
        this.loanBookingService.getAllLosCompletedApplicationsForDocumentation(body).subscribe((res) => {
    ...
    printOverviewInformation(): void {
        let printTitle = 'OVERVIEW INFORMATION';
        let printContents, popupWin;
        printContents = document.getElementById('print-section-overview-information').innerHTML;
      ...
      printCustomerInformation(): void {
        let printTitle = 'CUSTOMER INFORMATION';
        let printContents, popupWin;
        printContents = document.getElementById('print-section-customer-information').innerHTML;
        popupWin = window.open('', '_blank', 'top=0,left=0,height=100%,width=auto');
        popupWin.document.open();
        popupWin.document.write(`
        <html>
      ...
      printCorporateInformation(): void {
        let printTitle = 'OVERVIEW INFORMATION';
        let printContents, popupWin;
        printContents = document.getElementById('print-section-corporate-information').innerHTML;
        popupWin = window.open('', '_blank', 'top=0,left=0,height=100%,width=auto');
        popupWin.document.open();
        popupWin.document.write(`
        <html>
      ...
      printLoanInformation(): void {
        let printTitle = 'LOAN INFORMATION';
        let printContents, popupWin;
        printContents = document.getElementById('print-section-loan-information').innerHTML;
        popupWin = window.open('', '_blank', 'top=0,left=0,height=100%,width=auto');
        popupWin.document.open();
        popupWin.document.write(`
        <html>
    ...
    //                 __this.getAvailedApplicationsForDocumentation();
        ...
        this.loanBookingService.saveLoans(dataCollection)
            .subscribe((res) => {
                if (res.success === true) {
                    ...
                    this.initializeControls();
                    this.loadingSrv.hide();
                    //this.getAllLosCompletedApplicationsForDocumentation();
    ...
    // getAllLosCompletedApplicationsForDocumentation() {
    //     this.loadingSrv.show();
    //     this.loanBookingService.getAllLosCompletedApplicationsForDocumentation().subscribe((res) => {
    //         this.loanApplication = this.loans = res.result;
    //         this.loadingSrv.hide();
    //     }, (err) => {
    //         this.loadingSrv.hide();
    ...
    loadCustomerApplicationDetails(data) {
        ...
        // else {
            ...
            this.loanApplServ.getFinalOfferLetterByLoanAppId(this.loanSelection.loanApplicationId)
    ...
    previewDocumentation(print = false) {
      this.documentations = [];
      this.loadingSrv.show();
      this.camService.getDocumentation(this.OPERATION_ID, this.loanSelection.loanApplicationId).subscribe((response: any) => {
      this.documentations = response.result;
      this.loadingSrv.hide();
      if (print == false) this.displayDocumentation = true;
      else setTimeout(() => this.printFam(), 1000);
      }, (err) => {
          this.loadingSrv.hide(1000);
      });
  }
  ...
  printFam(): void {
    let printTitle = 'FACILITY APPROVAL MEMO No.:' + this.loanSelection.applicationReferenceNumber;
    let printContents, popupWin;
    ...
    flagPrinted(form) {
      ...
      let body = {
          ...
          customerName: form.customerName == null ? form.customerGroupName : form.customerName,
          customerCode: form.customerCode == null ? form.customerGroupCode : form.customerCode,
      ...
      promptMessage = 'Credit with Reference number: ' + form.applicationReferenceNumber + ' is flaged for filling.';
              swal({
                  ...
                  __this.loadingSrv.show();
                  __this.subscriptions.add(__this.camService.flagPrintedLos(body).subscribe((response: any) => {
                      if (response.success == true) {
                        swal(`${GlobalConfig.APPLICATION_NAME}`, response.message, 'success');
                          __this.displayCommentForm = false;
                          // __this.getAllLosCompletedApplicationsForDocumentation();
                          __this.loadingSrv.hide();
                      } else {
