...
import { LoanCovenantModel, ChargeFeeAppModel, LoanSchedule, LoanModel, LoanCollateralModel, monitoringTriggersModel, contingentLoanInputModel, revolvingLoanInputModel, FXLoanBeneficiaryModel } from 'app/credit/models';
...
import { SheduleComponent } from '..';
import { JobSource, ApprovalStatusEnum, DayCountConventionEnum, ProductTypeEnum, ProductClassEnum, GlobalConfig } from 'app/shared/constant/app.constant';
...
import { ExchangeRateService, AuthenticationService, AuthorizationService } from 'app/admin/services';
import { CreditAppraisalService, LoanApplicationService, CreditApprovalService, LoanService } from 'app/credit/services';
...
export class DisbursedLoansComponent implements OnInit {
  ...
  revolvingTypes: any;
  pagetitle: string ="Facilities Disbursed For:";
  basis: any;
  ...
  writeCustomerCasaAccount() {
      if ((this.loanSelection.customerAccounts != null || this.loanSelection.customerAccounts != undefined) && this.bookedFacilityData !== null) {       
        ...
        this.bookedFacilityData.casaAccountNumber = account !== null && account[0] !== undefined ? account[0].productAccountNumber : '';
        ...
        // var account2 = this.loanSelection.customerAccounts ? this.loanSelection.customerAccounts.filter(x=>x.casaAccountId == this.bookedFacilityData.casaAccountId2) : null;
        this.bookedFacilityData.casaAccountNumber2  = account2 !== null && account2[0] !== undefined ? account2[0].productAccountNumber : '';        
      }
  }
  ...
  getdisbursedLoans() {
      this.loadingSrv.show();
      this.loanBookingService.getdisbursedLoansApplicationDetails().subscribe((res) => {
        this.loadingSrv.hide();
  ...
  loadCustomerApplicationDetails(data) {
      ...
      if(this.loanSelection.productTypeId == ProductTypeEnum.Revolving) {
          ...
          if (this.bookedFacilityData !== null) {
  ...
  getLoanCurrenyPriceIndex(currencyId) { 
      this.productService.getProductPriceIndexByCurrencyId(currencyId).subscribe((data) => {
          if(data.success) { 
              ...
              var priceIndexId = this.bookedFacilityData !== null ? Number(this.bookedFacilityData.productPriceIndexId) : null;
