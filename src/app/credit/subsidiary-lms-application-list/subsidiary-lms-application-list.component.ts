...
import { ApprovalStatus, ApprovalStatusEnum, ProductClassProcessEnum } from 'app/shared/constant/app.constant';
import { LoadingService } from 'app/shared/services/loading.service';
import { LazyLoadEvent } from 'primeng/primeng';
import { CreditAppraisalService } from '../services';
import { AuthHttp, AuthenticationService } from 'app/admin/services';

@Component({
  selector: 'app-subsidiary-lms-application-list',
  templateUrl: './subsidiary-lms-application-list.component.html',
  styleUrls: ['./subsidiary-lms-application-list.component.scss']
})
export class SubsidiaryLmsApplicationListComponent implements OnInit {
  ...
  getLoanApplications( 
      ...
      if(this.isDrawdown === true && this.searchString != null && this.searchString != undefined){
          ...
          if(this.isCRMS ){
              ...
              this.camService.getReviewApplicationsSubsidiaries(page, itemsPerPage, this.operationId, classId, this.searchString).subscribe((response: any) => {
                  this.itemTotal = response.count; 
                  this.applications = response.result; 
                 //console.log("I am here "+JSON.stringify( this.applications ));
                  this.showLoadIcon = false;
  ...
  { 
    ...
    this.camService.setCountryCode(countryCode);
      ...
      if(this.operationId != 48){
              ...
              this.camService.getReviewApplicationsByID(this.applicationSelection.loanApplicationId).subscribe((response: any) => {
                  if(response.success){
