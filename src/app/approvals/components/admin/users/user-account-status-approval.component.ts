...
@Component({
    templateUrl: 'user-account-status-approval.component.html',
})

export class UserAccountStatusChangeApprovalComponent implements OnInit {

    usersFormGroup: FormGroup;
    displayUsersModal = false;
    usersAccountStatusApprovalData: any[] = [];
    ...
    ngOnInit(): void {
        ...
        this.getAllUserAccountStatusChangeAwaitingApproval(); this.getAllApprovalStatus();
    }

    getAllUserAccountStatusChangeAwaitingApproval(): void {
        this.adminService.getAllUserAccountStatusChangeAwaitingApproval().subscribe((response: any) => {
            this.usersAccountStatusApprovalData = response.result;
    ...
    viewUserDetails(index, evt) {
        ...
        this.selectedUserData = this.usersAccountStatusApprovalData[index];
    ...
    goForApproval(formObj, event) {
        ...
        swal({
            ...
            __this.adminService.sendForUserAccountStatusApproval(bodyObj).subscribe((response: any) => {
                __this.loadingService.hide();
                if (response.success === true) {
                    swal(`${GlobalConfig.APPLICATION_NAME}`, response.message, 'success');
                    __this.getAllUserAccountStatusChangeAwaitingApproval();
                } else {
