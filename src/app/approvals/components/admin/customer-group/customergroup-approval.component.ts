...
@Component({
    templateUrl: 'customergroup-approval.component.html'
})

export class CustomerGroupApprovalComponent implements OnInit {

    customergroupFormGroup: FormGroup;
    displayCustGrpModal = false;
    custGrpApprovalData: any[] = [];
    ...
    ngOnInit() {
        ...
        this.getAllCustGrpsAwaitingApproval(); this.getAllApprovalStatus();

    }

    getAllCustGrpsAwaitingApproval(): void {
        this.custGrpService.getAllCustomerGroupsAwaitingApproval().subscribe((response: any) => {
            this.custGrpApprovalData = response.result;
    ...
    viewCustGrpDetails(index, evt) {
        evt.preventDefault();
        this.selectedCustGrpData = {};
        this.selectedCustGrpData = this.custGrpApprovalData[index];
    ...
    goForApproval(formObj) {
        ...
        let bodyObj = {
            targetId: formObj.customerGroupId,
        ...
        swal({
            ...
            __this.custGrpService.sendCustomerGroupForApproval(bodyObj).subscribe((response: any) => {
                __this.loadingService.hide();
                if (response.success === true) {
                    swal(`${GlobalConfig.APPLICATION_NAME}`, response.message, 'success');
                    __this.getAllCustGrpsAwaitingApproval();
                } else {
