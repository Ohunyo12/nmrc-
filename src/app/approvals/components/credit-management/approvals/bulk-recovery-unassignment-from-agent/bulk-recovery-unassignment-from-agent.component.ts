...
@Component({
  selector: 'app-bulk-recovery-unassignment-from-agent',
  templateUrl: './bulk-recovery-unassignment-from-agent.component.html'
})
export class BulkRecoveryUnassignmentFromAgentComponent implements OnInit {
    ...
    getBulkRecoveryToAgentAwaitingApproval() {
        this.loadingService.show();
        this.loanOperationService.getBulkUnassignmentRecoveryFromAgentAwaitingApproval().subscribe((response: any) => {
    ...
    getAllLoansRecoveredByAgent(accreditedConsultantId,referenceId) {
      this.loanOperationService.getAllBulkUnassignLoansRecoveredByAgent(accreditedConsultantId,referenceId).subscribe((response: any) => {
    ...
    viewLoanDetails(row, evt) {
        ...
        this.getAllLoansRecoveredByAgent(this.loanSelectedData.accreditedConsultant,this.loanSelectedData.loanId);
        //this.getTrail23();
  ...
  promptToGoForBulkApproval(formObj){
    ...
    //console.log("i am testing it "+ bodyObj.comment);
    const __this = this;
    swal({
        title: 'Are you sure?',
        ...
        __this.loanOperationService.sendBulkUnassignRecoveryForBulkApproval(__this.bulkApprovalList,bodyObj.approvalStatusId,bodyObj.comment).subscribe((response: any) => {
    ...
    goForApproval(formObj) {
        ...
        swal({
            ...
            __this.loadingService.show();
            __this.loanOperationService.sendBulkUnassignmentRecoveryOperationForApproval(bodyObj).subscribe((response: any) => {
                __this.loadingService.hide();
                if (response.success === true) {
