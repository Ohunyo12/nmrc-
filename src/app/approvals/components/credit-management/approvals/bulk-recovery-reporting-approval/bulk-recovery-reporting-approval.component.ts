...
import { JobSource, GlobalConfig, ApprovalStatus } from 'app/shared/constant/app.constant';
...
@Component({
  selector: 'app-bulk-recovery-reporting-approval',
  templateUrl: './bulk-recovery-reporting-approval.component.html'
})
export class BulkRecoveryReportingApprovalComponent implements OnInit {
    ...
    previewDocumentation(print = false) {
        this.loadingService.show();
        this.camService.getDocumentation(this.OPERATION_ID, this.loanSelectedData.bulkRecoveryApprovalId).subscribe((response: any) => {
    ...
    getBulkRecoveryReportingAwaitingApproval() {
        this.loadingService.show();
        this.loanOperationService.GetBulkRecoveryReportingAwaitingApproval().subscribe((response: any) => {
        this.loanOperationApprovalData = response.result;
        this.loadingService.hide();
      });
    }

    getAllLoansRecoveryReportingByReference(referenceId) {
      this.loanOperationService.getAllLoansRecoveryReportingByReference(referenceId).subscribe((response: any) => {
    ...
    viewLoanDetails(row, evt) {
        ...
        this.lienRemovalId = this.loanSelectedData.bulkRecoveryApprovalId;
        this.lienRemovalOperationId = this.loanSelectedData.operationId;
        this.getAllLoansRecoveryReportingByReference(this.loanSelectedData.referenceId);
        this.getTrail23();
        this.getAllLoanRecoveryReportingDocuments(this.loanSelectedData.referenceId);
    ...
    goForApproval(formObj) {
        ...
        let bodyObj = {
            ...
            targetId: this.loanSelectedData.loanRecoveryReportApprovalId,
        ...
        swal({
            ...
            __this.loanOperationService.sendBulkRecoveryReportingForApproval(bodyObj).subscribe((response: any) => {
                __this.loadingService.hide();
                if (response.success === true) {
                  __this.loanSelectedData = {};
                    __this.loanSelectedData = {};
                    __this.getBulkRecoveryReportingAwaitingApproval();
    ...
    getAllUploadedOperationDocument() {
        ...
        let body = {
            ...
            loanReferenceNumber: this.loanSelectedData.loanReferenceNumber,
            databaseTable: 8,
            operationReviewId: this.loanSelectedData.appraisalOperationId,
        }
    ...
    getTrail() {
        ...
        this.subscriptions.add(
            this.camService.getTrailLms(this.loanSelectedData.loanRecoveryReportApprovalId, this.loanSelectedData.operationId).subscribe((response: any) => {
    ...
    referBackTrails(): any {
        this.backtrail = []; 
        this.loadingService.show();
        this.camService.getTrailForReferBack(this.loanSelectedData.loanRecoveryReportApprovalId, this.loanSelectedData.operationId, this.loanSelectedData.currentApprovalLevelId).subscribe((response: any) => {
            this.loadingService.hide();
        ...
        __this.reviewService.forwardLienRemovalApplication(body).subscribe((response: any) => {
            ...
            if (response.success == true) {
                ...
                if (response.stateId == 3) {
                     swal(`${GlobalConfig.APPLICATION_NAME}`, `The Lien Removal application will be <strong>${response.statusName}</strong>`, 'success'); 
                }
    ...
    displayApplicationStatusMessage(response) {
        if (response.stateId == 3) {
             swal(`${GlobalConfig.APPLICATION_NAME}`, `The lien removal application has been <strong>${response.statusName}</strong>`, 'success'); 
        }
        else
