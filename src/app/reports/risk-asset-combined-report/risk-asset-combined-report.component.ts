...
import { BranchService, ApprovalService } from 'app/setup/services';
...
@Component({
  selector: 'app-risk-asset-combined-report',
  templateUrl: './risk-asset-combined-report.component.html',
  styleUrls: ['./risk-asset-combined-report.component.scss']
})

export class RiskAssetCombinedReportComponent implements OnInit {
  PAGEOPERATION = 6; 
  ...
  auditTypeId: any;
  stagingSearch: any;
  RiskAssetCombinedReportForm: FormGroup;
  ...
  constructor(private approvalSer: ApprovalService, private loadingService: LoadingService,
    ...
    ngOnInit() {
    this.RiskAssetCombinedReportForm = this.fb.group({
  ...
  popoverSeeMore(form) {
    ...
    this.loadingService.show();
     ...
     var body = {
       ...
       this.reportServ.GetRiskAssetCombinedReport(body).subscribe((response: any) => {
  ...
  clearInput() {
    ...
    this.RiskAssetCombinedReportForm.setValue({
      misCode: null,
      level:  null,
      runDate: null,
      exposureType:null,
