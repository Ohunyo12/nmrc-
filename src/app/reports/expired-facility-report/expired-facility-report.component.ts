...
@Component({
  selector: 'app-expired-facility-report',
  templateUrl: './expired-facility-report.component.html'
  
})

export class ExpiredFacilityReportComponent implements OnInit {
  PAGEOPERATION = 6; 
  ...
  constructor(private approvalSer: ApprovalService, private loadingService: LoadingService,
    ...
    ngOnInit() {
    this.expiredFacilityForm = this.fb.group({
  ...
  popoverSeeMore(form) {
    ...
    this.loadingService.show();
     ...
     var body = {
       ...
       this.reportServ.GetExpiredFacilityReport(body).subscribe((response: any) => {
  ...
  clearInput() {
    ...
    this.expiredFacilityForm.setValue({
      misCode: null,
      level:  null,
      runDate: null,
      exposureType:null,
