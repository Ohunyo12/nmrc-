...
@Component({
  selector: 'app-unpaid-obligation-report',
  templateUrl: './unpaid-obligation-report.component.html',
  styleUrls: ['./unpaid-obligation-report.component.scss']
})

export class UnpaidObligationReportComponent implements OnInit {
  PAGEOPERATION = 6; 
  ...
  auditTypeId: any;
  stagingSearch: any;
  UnpaidObligationReportForm: FormGroup;
  ...
  constructor(private approvalSer: ApprovalService, private loadingService: LoadingService,
    ...
    ngOnInit() {
    this.UnpaidObligationReportForm = this.fb.group({
  ...
  popoverSeeMore(form) {
    ...
    this.loadingService.show();
     ...
     var body = {
       ...
       this.reportServ.GetUnpaidObligationReport(body).subscribe((response: any) => {
  ...
  clearInput() {
    ...
    this.UnpaidObligationReportForm.setValue({
      misCode: null,
      level:  null,
      runDate: null,
      exposureType:null,
