...
@Component({
  selector: 'app-maturity-report',
  templateUrl: './maturity-report.component.html',
  styleUrls: ['./maturity-report.component.scss']
})


export class MaturityReportComponent implements OnInit {
  PAGEOPERATION = 6; 
  ...
  constructor(private approvalSer: ApprovalService, private loadingService: LoadingService,
    ...
    ngOnInit() {
    this.maturityReportForm = this.fb.group({
  ...
  popoverSeeMore(form) {
    ...
    this.loadingService.show();
     ...
     var body = {
       ...
       this.reportServ.GetMaturityReport(body).subscribe((response: any) => {
  ...
  clearInput() {
    ...
    this.maturityReportForm.setValue({
      misCode: null,
      level:  null,
      runDate: null,
      exposureType:null,
