...
@Component({
  selector: 'app-risk-asset-contigent-report-main',
  templateUrl: './risk-asset-contigent-report-main.component.html',
  styleUrls: ['./risk-asset-contigent-report-main.component.scss']
})
export class RiskAssetContigentReportMainComponent implements OnInit {
  PAGEOPERATION = 6; 
  ...
  auditTypeId: any;
  stagingSearch: any;
  RiskAssetContigentReportMainForm: FormGroup;
  ...
  constructor(private approvalSer: ApprovalService, private loadingService: LoadingService,
    ...
    ngOnInit() {
    this.RiskAssetContigentReportMainForm = this.fb.group({
  ...
  popoverSeeMore(form) {
    ...
    this.loadingService.show();
     ...
     var body = {
       ...
       this.reportServ.GetRiskAssetContigentReportMain(body).subscribe((response: any) => {
  ...
  clearInput() {
    ...
    this.RiskAssetContigentReportMainForm.setValue({
      misCode: null,
      level:  null,
      runDate: null,
      exposureType:null,
