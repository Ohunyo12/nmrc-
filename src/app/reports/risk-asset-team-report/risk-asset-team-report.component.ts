...
@Component({
  selector: 'app-risk-asset-team-report',
  templateUrl: './risk-asset-team-report.component.html',
  styleUrls: ['./risk-asset-team-report.component.scss']
})

export class RiskAssetTeamReportComponent implements OnInit {
  PAGEOPERATION = 6; 
  ...
  constructor(private approvalSer: ApprovalService, private loadingService: LoadingService,
    ...
    ngOnInit() {
    this. RiskAssetTeamForm = this.fb.group({
  ...
  popoverSeeMore(form) {
    ...
    this.loadingService.show();
     ...
     var body = {
       ...
       this.reportServ.GetRiskAssetTeamReport(body).subscribe((response: any) => {
  ...
  clearInput() {
    ...
    this.RiskAssetTeamForm.setValue({
      misCode: null,
      level:  null,
      runDate: null,
      exposureType:null,
