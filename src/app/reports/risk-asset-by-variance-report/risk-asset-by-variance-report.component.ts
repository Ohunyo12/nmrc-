...
@Component({
  selector: 'app-risk-asset-by-variance-report',
  templateUrl: './risk-asset-by-variance-report.component.html',
  styleUrls: ['./risk-asset-by-variance-report.component.scss']
})


export class RiskAssetByVarianceReportComponent implements OnInit {
  PAGEOPERATION = 6; 
  ...
  auditTypeId: any;
  stagingSearch: any;
  RiskAssetByVarianceReportForm: FormGroup;
  ...
  constructor(private approvalSer: ApprovalService, private loadingService: LoadingService,
    ...
    ngOnInit() {
    this.RiskAssetByVarianceReportForm = this.fb.group({
  ...
  popoverSeeMore(form) {
    ...
    this.loadingService.show();
     ...
     var body = {
       ...
       this.reportServ.GetRiskAssetByVarianceReport(body).subscribe((response: any) => {
  ...
  clearInput() {
    ...
    this.RiskAssetByVarianceReportForm.setValue({
      misCode: null,
      level:  null,
      runDate: null,
      exposureType:null,
