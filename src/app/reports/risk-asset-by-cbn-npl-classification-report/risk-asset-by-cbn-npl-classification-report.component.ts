...
@Component({
  selector: 'app-risk-asset-by-cbn-npl-classification-report',
  templateUrl: './risk-asset-by-cbn-npl-classification-report.component.html',
  styleUrls: ['./risk-asset-by-cbn-npl-classification-report.component.scss']
})


export class RiskAssetByCbnNplClassificationReportComponent  implements OnInit {
  PAGEOPERATION = 6; 
  ...
  auditTypeId: any;
  stagingSearch: any;
  riskAssetByCbnReportForm: FormGroup;
  ...
  constructor(private approvalSer: ApprovalService, private loadingService: LoadingService,
    ...
    ngOnInit() {
    this.riskAssetByCbnReportForm = this.fb.group({
  ...
  popoverSeeMore(form) {
    ...
    this.loadingService.show();
     ...
     var body = {
       ...
       this.reportServ.GetRiskAssetByCbnNplClassificationReport(body).subscribe((response: any) => {
  ...
  clearInput() {
    ...
    this.riskAssetByCbnReportForm.setValue({
      misCode: null,
      level:  null,
      runDate: null,
      exposureType:null,
