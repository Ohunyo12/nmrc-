...
@Component({
  selector: 'app-contigent-liability-report',
  templateUrl: './contigent-liability-report.component.html',
  styleUrls: ['./contigent-liability-report.component.scss']
})

export class ContigentLiabilityReportComponent  implements OnInit {
  PAGEOPERATION = 6; 
  ...
  auditTypeId: any;
  stagingSearch: any;
  contigentLiabilityReportForm: FormGroup;
  ...
  constructor(private approvalSer: ApprovalService, private loadingService: LoadingService,
    ...
    ngOnInit() {
    this.contigentLiabilityReportForm = this.fb.group({
  ...
  popoverSeeMore(form) {
    ...
    this.loadingService.show();
     ...
     var body = {
       ...
       this.reportServ.GetContigentLiabilityReport(body).subscribe((response: any) => {
  ...
  clearInput() {
    ...
    this.contigentLiabilityReportForm.setValue({
      misCode: null,
      level:  null,
      runDate: null,
      exposureType:null,
