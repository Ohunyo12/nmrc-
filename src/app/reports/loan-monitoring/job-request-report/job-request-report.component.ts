...
@Component({
  selector: 'app-job-request-report',
  templateUrl: './job-request-report.component.html'
})
export class JobRequestReportComponent implements OnInit {
  ...
  popoverSeeMore() {
    ...
    if (this.startDate != null && this.endDate != null) {
      ...
      //console.log('>>>>>>>>>>',data);203056491
     /// console.log('>>>>>>>>>>');
      this.reportServ.GetJobRequestReport(data).subscribe((response: any) => {
  ...
  searchForAuditTypName(auditTypeName) {
    this.loadingService.show();
    this.reportServ.SearchForAuditType(auditTypeName).subscribe((response: any) => {
