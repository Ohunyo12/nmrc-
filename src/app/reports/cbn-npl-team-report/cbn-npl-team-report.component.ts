...
@Component({
  selector: 'app-cbn-npl-team-report',
  templateUrl: './cbn-npl-team-report.component.html',
  styleUrls: ['./cbn-npl-team-report.component.scss']
})

export class CbnNplTeamReportComponent implements OnInit {
  PAGEOPERATION = 6; 
  ...
  auditTypeId: any;
  stagingSearch: any;
  cbnNplTeamReportForm: FormGroup;
  ...
  constructor(private approvalSer: ApprovalService, private loadingService: LoadingService,
    ...
    ngOnInit() {
    this.cbnNplTeamReportForm = this.fb.group({
  ...
  popoverSeeMore(form) {
    ...
    this.loadingService.show();
     ...
     var body = {
       ...
       this.reportServ.GetcbnNplTeamReport(body).subscribe((response: any) => {
  ...
  clearInput() {
    ...
    this.cbnNplTeamReportForm.setValue({
      misCode: null,
      level:  null,
      runDate: null,
      exposureType:null,
